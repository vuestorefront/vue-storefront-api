...
- (A) `yarn restore`
- (B) `docker exec -it vue-storefront-api-app-1 yarn restore`

Then, to update the structures in the database to the latest version (data migrations), do the following:

- (A) `yarn migrate`
- (B) `docker exec -it vue-storefront-api-app-1 yarn migrate`
...
- (A) `yarn dump`
- (B) `docker exec -it vue-storefront-api-app-1 yarn dump`
...
Executing `docker exec -it vue-storefront-api-app-1 yarn install` will also download your custom modules dependencies.
