    ...
    es.search(common.db, {
        ...
        for (const hit of hits) {
            ...
            if (cli.options.oldFormat) {
                console.log('/%s /p/%s/%s/%s;', product.url_key, decodeURIComponent(product.sku), product.url_key, decodeURIComponent(product.sku))
            } else {
                console.log('/%s /%s/%s;', product.url_key, product.url_key, decodeURIComponent(product.sku))
            }
        ...
        es.search(common.db, {
            ...
            for (const hit of hits) {
                ...
                if (cli.options.oldFormat) {
                    console.log('/%s /c/%s;', category.url_path, category.url_key)
                } else {
                    console.log('/%s /%s;', category.url_path, category.url_key)
                }    
            }
        })        
    })
